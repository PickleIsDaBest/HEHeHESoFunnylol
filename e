--Bro rlly spent 5 minutes just to get a script that ain't even mine bozo.
...
if eq(time3,tostring(Base64.Decode(res1.data)),true) then
	...
	local Window = library:Window("FrostyHub", " - Blox Burg")
	...
	if game:HttpGet("https://bloxburg.sexy/version.php") ~= tostring(ReplicatedStorage.Version.Value) then 
		Window:Notification("Error", {
			Text = "Only Autofarm for 30 minutes to be safe :)";
			ConfirmText = "Okay";
		})
	end
